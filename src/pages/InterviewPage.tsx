...
import { MOCK_CANDIDATES, Candidate } from '@/lib/constants';
export function InterviewPage() {
  ...
  useEffect(() => {
    const foundCandidate = MOCK_CANDIDATES.find(c => c.id === candidateId);
    if (foundCandidate) {
      setCandidate(foundCandidate);
      chatService.newSession();
      setChatState(prev => ({ ...prev, messages: [], sessionId: chatService.getSessionId() }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!input.trim() || isLoading || chatState.isProcessing) return;
